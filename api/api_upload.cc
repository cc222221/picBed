...
/**
 * ./x86_64-linux-gnu/sys/syslog.h:57:#define      LOG_INFO
 *  fdfs_client.h 进一步包含的头文件里面有LOG_INFO，导致和项目里的有冲突
 */
#include "fdfs_client.h"

#undef LOG_INFO // 这里的目的取消x86_64-linux-gnu/sys/syslog.h:57
                // :#define LOG_INFO的定义，使用我们工程文件的定义
...
int getFullurlByFileid(char *fileid, char *fdfs_file_url) {
    ...
    } else //父进程
    {
        ...
        LOG_INFO << "host_name:" << s_storage_web_server_ip
                 << ", fdfs_file_host_name: " << fdfs_file_host_name;
...
int ApiUpload(string &url, string &post_data, string &str_json) {
    ...
    //拷贝分界线
    strncpy(boundary, begin,
            p1 - begin); // 缓存分界线, 比如：WebKitFormBoundary88asdgewtgewx
    ...
    if (ret < 0) {
        LOG_ERROR << "rename " << file_path << " to " << new_file_path
                  << "failed";
        ret = -1;
        goto END;
    }
    //===============> 将该文件存入fastDFS中,并得到文件的file_id <============
    LOG_INFO << "uploadFileToFastDfs, file_name:" << file_name
    ...
    if (ret != 0) {
        LOG_WARN
            ...
            << " failed"; // 删除失败则需要有个监控重新清除过期的临时文件，比如过期两天的都删除
    }
    ...
    str_json =
        value
            .toStyledString(); // json序列化,
                               // 直接用writer是紧凑方式，这里toStyledString是格式化更可读方式
